...
def modify_csv(file_path, variation, output_file):
    ...
    df["Rain (mm)"] = df["Rain (mm)"] * (1 + variation)
    
    df["Rain (mm)"] = df["Rain (mm)"].where(df["Rain (mm)"] >= 0, 0)
...
if __name__ == '__main__':
    ...
    for variation in variations:
        ...
        OUTPUT_FILENAME = os.path.join(results_dir, f"variation_P_{int(variation * 100)}.csv")
        ...
        t2 = time.time()
        print(f'Variación P {variation * 100}% completada en {t2-t1:.2f} segundos')
