...
def modify_csv(file_path, variation, output_file):
    ...
    df["Relative Humidity %"] = df["Relative Humidity %"] * (1 + variation)
    
    # Asegurar que el valor de RH esté entre 0 y 100 
    df["Relative Humidity %"] = df["Relative Humidity %"].apply(lambda x: 0 if x < 0 else (100 if x > 100 else x))
...
if __name__ == '__main__':
    ...
    for variation in variations:
        ...
        OUTPUT_FILENAME = os.path.join(results_dir, f"variation_RH_{int(variation * 100)}.csv")
        ...
        t2 = time.time()
        print(f'Variación RH {variation * 100}% completada en {t2-t1:.2f} segundos')
